...
// Charge lines that appear in the order sheets but are not physical stock.
// They must never be listed, stock-checked or dispatched against batches.
const NON_STOCK_SKUS = new Set(['COD-FEE']);

export function isNonStockSku(sku) {
  return NON_STOCK_SKUS.has(String(sku || '').trim().toUpperCase());
}
...
export function parseOrderItems(rows, orderId, comboMap, headerMap) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (rowOrderId !== orderId || qty <= 0 || isNonStockSku(sku)) continue;
...
export function parseProductRowsByOrder(rows, comboMap, headerMap) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!orderId || qty <= 0 || isNonStockSku(sku)) continue;
