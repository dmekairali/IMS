// lib/googleSheets.js - UPDATED VERSION - Replace your existing file
import { google } from 'googleapis';

let sheetsInstance = null;
let batchCacheData = null;
let batchCacheTimestamp = null;
const CACHE_TTL = 50 * 60 * 1000; // 5 min
...
export function getBatchCache() {
  ...
  if (batchCacheData && batchCacheTimestamp && (now - batchCacheTimestamp) < CACHE_TTL) {
    const remainingSeconds = Math.round((CACHE_TTL - (now - batchCacheTimestamp)) / 1000);
    console.log(`📦 Returning server-side cached data (expires in ${remainingSeconds}s)`);
    return batchCacheData;
  }
  console.log('⏰ Server-side cache expired or empty');
  return null;
}
...
export function setBatchCache(data) {
  ...
  console.log(`💾 Server-side cache set with ${data.length} batches (TTL: ${CACHE_TTL / 1000}s)`);
}
